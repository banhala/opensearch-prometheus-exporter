...
public class RestPrometheusMetricsAction extends BaseRestHandler {
    ...
    private final KnnStatsCache knnStatsCache;
    ...
    public RestPrometheusMetricsAction(Settings settings, ClusterSettings clusterSettings) {
        ...
        this.localScheme = settings.getAsBoolean("plugins.security.ssl.http.enabled", false) ? "https" : "http";
        this.knnStatsCache = new KnnStatsCache(30_000L);
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) {
        ...
        // Capture request context before the async lambda (RestRequest may be recycled)
        final int port = request.getHttpChannel().getLocalAddress().getPort();
        ...
        return channel -> client.execute(INSTANCE, metricsRequest,
                new RestResponseListener<NodePrometheusMetricsResponse>(channel) {
                    ...
                    public RestResponse buildResponse(NodePrometheusMetricsResponse response) throws Exception {
                        ...
                        boolean isPrometheusKnn = prometheusSettings.getPrometheusKnn();

                        // Fetch KNN stats if enabled (before collector to keep try-catch clean)
                        KnnStatsData knnStatsData = null;
                        if (isPrometheusKnn) {
                            knnStatsCache.updateContext(
                                    localScheme, port, authHeader, prometheusSettings.getNodesFilter());
                            knnStatsData = knnStatsCache.getOrFetch();
                        }
...
//                            PrometheusMetricsCatalog catalog = new PrometheusMetricsCatalog(clusterName, nodeName, nodeId, metricPrefix);
                            ...
                            if (isPrometheusKnn) {
                                collector.updateKnnMetrics(knnStatsData);
                            }
